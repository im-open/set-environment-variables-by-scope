    ...
    var Collection = class extends Node.NodeBase {
      ...
      addIn(path, value) {
        if (isEmptyPath(path)) this.add(value);
          ...
          if (Node.isCollection(node)) node.addIn(rest, value);
      ...
      getIn(path, keepScalar) {
        const [key, ...rest] = path;
        const node = this.get(key, true);
        if (rest.length === 0) return !keepScalar && Node.isScalar(node) ? node.value : node;
        else return Node.isCollection(node) ? node.getIn(rest, keepScalar) : void 0;
      }
      ...
      setIn(path, value) {
        ...
        if (rest.length === 0) {
          ...
          const node = this.get(key, true);
          if (Node.isCollection(node)) node.setIn(rest, value);
          else if (node === void 0 && this.schema) this.set(key, collectionFromPath(this.schema, rest, value));
          else throw new Error(`Expected YAML collection at ${key}. Remaining path: ${rest}`);
        }
      }
    ...
    function stringifyPair({ key, value }, ctx, onComment, onChompKeep) {
      ...
      let explicitKey =
        ...
        (!key ||
          ...
          (Node.isScalar(key) ? key.type === Scalar.Scalar.BLOCK_FOLDED || key.type === Scalar.Scalar.BLOCK_LITERAL : typeof key === 'object'));
  ...
  'node_modules/yaml/dist/nodes/addPairToJSMap.js'(exports2) {
    ...
    function addPairToJSMap(ctx, map, { key, value }) {
      if (ctx?.doc.schema.merge && isMergeKey(key)) {
        ...
        if (Node.isSeq(value)) for (const it of value.items) mergeToJSMap(ctx, map, it);
    ...
    var YAMLMap = class extends Collection.Collection {
      ...
      add(pair, overwrite) {
        ...
        if (prev) {
          ...
          if (Node.isScalar(prev.value) && Scalar.isScalarValue(_pair.value)) prev.value.value = _pair.value;
  ...
  'node_modules/yaml/dist/schema/common/map.js'(exports2) {
    ...
    function createMap(schema, obj, ctx) {
      ...
      const add = (key, value) => {
        ...
        if (value !== void 0 || keepUndefined) map2.items.push(Pair.createPair(key, value, ctx));
      };
    ...
    var map = {
      ...
      resolve(map2, onError) {
        if (!Node.isMap(map2)) onError('Expected a mapping for this tag');
        return map2;
      }
    };
    exports2.map = map;
  ...
  'node_modules/yaml/dist/nodes/YAMLSeq.js'(exports2) {
    ...
    var YAMLSeq = class extends Collection.Collection {
      ...
      set(key, value) {
        ...
        if (Node.isScalar(prev) && Scalar.isScalarValue(value)) prev.value = value;
  ...
  'node_modules/yaml/dist/schema/common/seq.js'(exports2) {
    ...
    var seq = {
      ...
      resolve(seq2, onError) {
        if (!Node.isSeq(seq2)) onError('Expected a sequence for this tag');
        return seq2;
      }
    };
    exports2.seq = seq;
    ...
    function mapIncludes(ctx, items, search) {
      ...
      const isEqual =
        typeof uniqueKeys === 'function'
          ? uniqueKeys
          : (a, b) => a === b || (Node.isScalar(a) && Node.isScalar(b) && a.value === b.value && !(a.value === '<<' && ctx.schema.merge));
...
var require_compose_scalar = __commonJS({
  'node_modules/yaml/dist/compose/compose-scalar.js'(exports2) {
    ...
    function findScalarTagByName(schema, value, tagName, tagToken, onError) {
      if (tagName === '!') return schema[Node.SCALAR];
      const matchWithTest = [];
  ...
  'node_modules/yaml/dist/parse/cst-scalar.js'(exports2) {
    ...
    function setBlockScalarValue(token, source) {
      ...
      if (token.type === 'block-scalar') {
        ...
        const props = [{ type: 'block-scalar-header', offset, indent, source: head }];
        if (!addEndtoBlockProps(props, 'end' in token ? token.end : void 0)) props.push({ type: 'newline', offset: -1, indent, source: '\n' });
...
var require_cst = __commonJS({
  'node_modules/yaml/dist/parse/cst.js'(exports2) {
    ...
    var isScalar = token =>
      !!token &&
      (token.type === 'scalar' || token.type === 'single-quoted-scalar' || token.type === 'double-quoted-scalar' || token.type === 'block-scalar');
...
var input_scope = core.getInput('scope', { required: true });
