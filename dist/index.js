...
var require_action_library = __commonJS({
  "src/action_library.js"(exports2, module2) {
    ...
    var action_library2 = class {
      ...
      filterObjectProperties = (filterObject, filterFunction, keyTransform) => Object.keys(filterObject).filter((k) => filterFunction(k)).reduce((scoped, key) => {
      ...
      keyIsPartOfScope = (scope, key) => {
        const scopes = key.split("@")[1].split(" ").map((s) => s.toUpperCase());
        return scopes.includes(scope.toUpperCase());
      };
      buildAllKeysUsed = (newItems) => Object.keys(newItems).reduce((keys, key) => {
        return {
          ...keys,
